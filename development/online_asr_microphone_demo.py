"""
Online ASR Microphone Demo - Cache-Aware Streaming
Converted from NeMo tutorial notebook to standalone Python script

This script allows you to do real-time ("streaming") speech recognition 
using audio recorded from your microphone with NeMo chunk-aware FastConformer models.
"""
...
class OnlineASR:
    def __init__(self, model_name, lookahead_size=80, decoder_type="rnnt"):
        """
        Initialize the Online ASR system
        
        Args:
            model_name: Name of the pretrained model to use
            lookahead_size: Lookahead size in milliseconds (0, 80, 480, or 1040 for multi model)
    ...
    def _setup_model(self):
        ...
        print(f"Using decoder: {self.decoder_type}")
    ...
    def transcribe_chunk(self, audio_chunk):
        ...
        return final_transcriptions[0] if final_transcriptions else ""
...
def run_streaming_asr(asr_system, device_id=None, chunk_size_ms=None):
    """
    Run streaming ASR with microphone input
    
    Args:
        asr_system: OnlineASR instance
    ...
    try:
        ...
        def stream_callback(in_data, frame_count, time_info, status):
            ...
            text = asr_system.transcribe_chunk(signal)
            
            # Print transcription (overwrite previous line)
            if text.strip():
...
def main():
    parser = argparse.ArgumentParser(
        description="Online ASR Microphone Demo with Cache-Aware Streaming",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Available models:
{chr(10).join(f"  - {model}" for model in AVAILABLE_MODELS)}

For multi model, lookahead can be: 0, 80, 480, or 1040 ms
For single-lookahead models, use the lookahead specified in the model name.

Examples:
  python {__file__} --model stt_en_fastconformer_hybrid_large_streaming_multi --lookahead 80
  python {__file__} --model stt_en_fastconformer_hybrid_large_streaming_480ms --decoder ctc
        """
    )
    ...
    try:
        ...
        print("Initializing Online ASR system...")
        asr_system = OnlineASR(
